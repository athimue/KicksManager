...
class StatisticsViewModel @Inject constructor(
    ...
    val uiState = getSellsStatisticsUseCase.invoke()
        .map {
            StatisticsUiState(
                shoes = 15,
                sells = it.size,
                ...
                vintedSells = it.filter { sell -> sell.sellPlace == "VINTED" }
                    .map { sell -> sell.sellPrice }
                    .fold(0.0) { acc, profitItem -> acc + profitItem },
                goatSells = it.filter { sell -> sell.sellPlace == "ALIAS" }
                    .map { sell -> sell.sellPrice }
                    .fold(0.0) { acc, profitItem -> acc + profitItem },
                meetupSells = it.filter { sell -> sell.sellPlace == "MEET_UP" }
                    .map { sell -> sell.sellPrice }
                    .fold(0.0) { acc, profitItem -> acc + profitItem },
                beforeShopSells = it.filter { sell -> sell.sellPlace == "BEFORE_SHOP" }
                    .map { sell -> sell.sellPrice }
                    .fold(0.0) { acc, profitItem -> acc + profitItem },
            )
        }
